...
## Name the geometry you want to inspect
...
A measurement selector must match exactly one item. An ambiguous match is an
error, not permission to choose the first edge. Circle centers and radii come
from native curves, including arcs; they are not inferred from mesh vertices.

## State requirements and read the result
...
## Find unwanted material and interference
...
area for faces, and length for curves. Its tolerance uses those units. To catch a
supported feature, inspect its bottom face against the supporting body: any exposed
area is useful evidence even when the final union is a valid solid.
...
curved faces; they are intentionally stronger than comparing total volumes.

## Put measured dimensions on a drawing
...
{:ok, drawing} = Drawing.dimension(drawing, diameter, offset: 5)
...
that support skills. Documentation and skills help an agent choose the tools;
the numerical checks provide evidence that its model meets the requirements.
