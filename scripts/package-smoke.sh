...
cp "$root/smith-0.3.0.tar" "$work/registry/tarballs/"
if [ -n "${OCEX_ARCHIVE:-}" ]; then
  cp "$OCEX_ARCHIVE" "$work/registry/tarballs/ocex-0.3.0.tar"
else
  (cd "$work/registry/tarballs" && mix hex.package fetch ocex 0.3.0)
fi
